...
class PlatformFFI {
  ...
  void _startListenEvent(YIKEImpl YIKEImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = YIKEImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
